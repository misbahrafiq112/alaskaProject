...
export const login = async (req, res) => {
  try {
    ...
    const hashedPass = await bcrypt.compare(
      password,
      matchedUser.userSecret.password
    );
...
export const logOut = async (req, res) => {
  try {
    ...
    jwt.verify(
      ...
      async (err) => {
        if (err) {
          ...
          return res.status(403).json({ message: "token didn't not match" });
        }
    ...
    console.log("Sessions logged out:",deletedSessions);
    return res
      .status(200)
      .json({ msg: "Sessions successfully logged out", deletedSessions });
  } catch (error) {
...
export const refreshToken = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "accessToken generate successfully", accessToken });
  } catch (error) {
    return res.status(500).json({ msg: "Server Error", Error: error.message });
  }
};
...
export const changePassword = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Password updated successfully", updatedPassword });
  } catch (error) {
